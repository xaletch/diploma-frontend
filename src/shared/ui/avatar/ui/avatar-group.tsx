      ...
      <Card>
        ...
        <CardContent className="space-y-5 pt-0">
          <div className="flex -space-x-2.5">
            {data.slice(0, 6).map((item, idx) => <Avatar key={idx} id={item.id} avatar_url={item.avatar} name={item.name} size={"small"} className={"rounded-full ring-2 ring-card-ring text-xs"} />)}
